...
driveCommand::driveCommand() {
	...
	LOOP_IT_CHECK = 5; //checks for iterations through loop. Change later depending on robot

	IMU_Scaled = 0; // Translated to a non-wrapping scale (-180 to 180 -> 820 to 1180)

	RotateAxisOverride = 0; //Joystick value

	setSpeed = 0; // Saves the rotation speed functional parameter passed in.
	setAngle = 0; // Saves the commanded go-to angle functional parameter passed in.

	setinitSpeed = 0; // Initial Set Speed
	setinitAngle = 0; // Initial Set Angle

	degrees_to = 0; //degrees to rotate to "Rotato" - Walter 2014
	degrees_to_abs = 0; //absolute value of degrees_to
	...
	RotCmd = 0; // Direction * Rot Speed passed to drive command.
	RotCmdInDrive = 0; //Same as above except for drive command
	...
	isDone = false; //Flag to OS/WPI to end this command.
	rotateIsDone = true; //Rotate command is done

	times_through_loop = 0; //counter for fixing overshooting

	button2_pressed = false; //B button on xbox 360 controller
}
